...
\def\onepageout#1{%
 \shipout\vbox{%方框
  \ifframe%
    ...
    \ifodd\pageno \m=1 %
    \else\ifnum\pageno=2 \m=1 %第二頁是特例，用於不想用四重方框時
    \else\m=0\fi\fi %
    \ifnum\m=1 % 內套的三个分开的方框
        \ruleht=1pt %
        \hbox{ % 内含三个框
            \kern-12.8pt %
            \vbox to 2pt{% 左小框
                ...
                \hbox to 36pt{%
                    \vrule width\ruleht height\pboxheight%
                    \hfil\kern16pt\leftpagehdr\hfil % 左页眉文字
                    \vrule width\ruleht height\pboxheight%
                }%
...
\def\putpagehdr{%放頁眉，用於P2,P3
  \vbox to 2cm{\vskip-1.9cm\hbox to 0pt{%
    \orotate{\hbox to 3.6cm{%
      \pre_phdr\hfil%
      \ifnum\pageno=2 གཅིག %
...
\def\modulo#1#2{ % 求余数
    \tmpcnta=#1 %
    \divide\tmpcnta by#2 \multiply\tmpcnta by#2 \multiply\tmpcnta by-1 %
    \advance\tmpcnta by #1 \ti=\tmpcnta}
%用于设置从P5开始的正文段落形状的宏
\def\slineil{35pt\dr}%
...
\def\setparshape#1#2{%{5/6}{该段文字大致最多的页数(<32*2)}
    ...
    \dr=\hsize \advance\dr by-2.4cm \dt=\hsize %
    \ifnum#1=5 %
...
\def\setnewparshape#1{ %根據上一段剩餘的行數設置新一段的parshape
    ...
    %\showthe\ti \showthe\tj %
    \modulo{\the\ti}{\the\tj} \prevlinesremained=\ti %
